...
use crate::models::interpretation::{
    Block, ComparisonRow, Concept, Interpretation, QuizOption, Stat, TimelineItem,
};
use crate::prompt;
...
impl Interpreter {
    ...
    /// 对一篇论文执行完整解读
    pub async fn interpret(&self, paper_id: Uuid, title: &str, text: &str) -> AppResult<Interpretation> {
...
impl Block {
    fn from_raw(r: RawBlock) -> AppResult<Block> {
        ...
        match r.btype.as_str() {
            "section" => Ok(Block::Section { id, num: r.num, title: r.title }),
            "paragraph" => Ok(Block::Paragraph { id, text: r.text }),
            "quote" => Ok(Block::Quote { id, text: r.text, cite: r.cite }),
            "stat_row" => Ok(Block::StatRow {
                id,
                stats: r.stats.into_iter().map(|s| Stat { value: s.value, label: s.label }).collect(),
            }),
            ...
            "timeline" => Ok(Block::Timeline {
                id,
                items: r.items.into_iter().map(|i| TimelineItem { year: i.year, title: i.title, body: i.body }).collect(),
            }),
            "comparison" => Ok(Block::Comparison {
                id,
                columns: r.columns,
                rows: r.rows.into_iter().map(|r| ComparisonRow { label: r.label, cells: r.cells }).collect(),
            }),
            "quiz" => Ok(Block::Quiz {
                id,
                question: r.question,
                options: r.options.into_iter().map(|o| QuizOption { text: o.text, correct: o.correct }).collect(),
                explain: r.explain,
            }),
            "code_fragment" => Ok(Block::CodeFragment { id, lang: r.lang, code: r.code }),
            "custom_html" => Ok(Block::CustomHtml { id, html: r.html }),
            other => Err(AppError::InvalidLlmOutput(format!("未知的 block 类型: {other}"))),
        }
    }
}
